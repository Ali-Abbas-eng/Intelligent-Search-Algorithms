...
class MiniMaxSearch(Player):
    ...
    def max_value(self, state, alpha, beta):
        ...
        for action in self.game.get_actions(state=state):
            value = max(value, self.min_value(self.game.get_result(state=state, action=action), alpha, beta))
            if(value >= beta):
                return value
    ...
    def min_value(self, state, alpha, beta):
        ...
        value = Inf
        for action in self.game.get_actions(state=state):
            value = min(value, self.game.get_result(state=state, action=action), alpha, beta)
            if(value <= alpha):
                return value
            beta = min(beta, value)
        return value
